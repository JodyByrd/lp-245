...
int hit_player(int dam) 
{
    ...
    else if (!alt_attacker_ob && attacker_ob != this_player() && this_player() != this_object())
...
int attack()
{
    ...
    if (!name_of_attacker || name_of_attacker == NAME_OF_GHOST || environment(attacker_ob) != environment(this_object())) 
	{
		if (!hunter && name_of_attacker && !attacker_ob->query_ghost())
		{
	    	tell_object(this_object(), "You are now hunting " + attacker_ob->query_name() + ".\n");
	    	hunted = attacker_ob;
	    	hunting_time = 10;
		}
		attacker_ob = 0;
		...
		{
	    	if (attacker_ob)
				tell_object(this_object(), "You turn to attack " + attacker_ob->query_name() + ".\n");
	    	return 1;
		}
		return 0;
    }
    ...
    if(name_of_weapon) 
	{
		...
		if (!attacker_ob) 
		{
	    	...
	    	log_file("BAD_SWORD", name_of_weapon->short() + ", " + creator(name_of_weapon) + " XX !\n");
	    	spell_cost = 0;
	    	spell_dam = 0;
    ...
    if(whit != "miss") 
	{
		...
		tmp = 0;
    tmp -= spell_dam;
    if (!is_npc && name_of_weapon && tmp > 20 && random(100) < weapon_class - level * 2 / 3 - 14) 
	{
		tell_object(this_object(), "CRACK!\nYour weapon broke!\n");
		tell_object(this_object(), "You are too inexperienced for such a weapon.\n");
    ...
    if (attacker_ob && attacker_ob->query_name() != NAME_OF_GHOST) 
	{
		...
		tell_object(this_object(), "You " + what + " " + name_of_attacker + how + ".\n");

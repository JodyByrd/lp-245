...
void show_scar() 
{
    ...
    while(i < MAX_SCAR) 
    {
        if (scar & j) 
        {
            ...
            if (first) 
            {
                write(cap_name + " has a scar on " + query_possessive() + " " + scar_desc[i]);
                first = 0;
            } 
            else if (old_value) 
            {
...
int score(string arg)
{
    ...
    write("You have " + experience + " experience points, " + money + " gold coins, ");
...
static void new_password(string p)
{
    ...
    "room/AdventurersGuild/adv_guild"->advance(0);
...
static void move_player_to_start3(mixed where) {
    ...
    if (where)
        ...
        move_object(myself, "room/Church/church");
    ...
    //
    if (level >= 21)
        cat("/News/WizardNews.txt");
    
    "room/PostOffice/PostOffice"->query_mail();

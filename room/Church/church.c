...
void long(string str)
{
    if (str == "clock") 
	{
		...
		if (this_player()->query_level() < 20)
	    	return;
		write("Time since reset is " + (time() - time_from_reset) + " seconds.\n");
		if (last_reset_cycle)
    ...
    if (str == "door") 
	{
		if (!"room/Church/elevator"->query_door(0) && "room/Church/elevator"->query_level(0))
...
int open(string str)
{
    if (str != "door")
		return 0;
    if ("room/Church/elevator"->query_level(0) != 2) 
	{
		write("You can't when the elevator isn't here.\n");
		return 1;
    }
    "room/Church/elevator"->open_door("door");
    return 1;
}
...
int close(string str)
{
    if (str != "door")
		return 0;
    "room/Church/elevator"->close_door("door");
    return 1;
}
...
int south() 
{
    this_player()->move_player("south#room/village/green");
    return 1;
}

int west() 
{
    if ("room/Church/elevator"->query_door(0) ||
		"room/Church/elevator"->query_level(0) != 2) 
	{
		write("The door is closed.\n");
		return 1;
    }
    this_player()->move_player("west#room/Church/elevator");
    return 1;
}

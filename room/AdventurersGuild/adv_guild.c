...
void extra_reset(int arg) 
{
    ...
    move_object("obj/Book/book", this_object());

    move_object(clone_object("obj/Bulletin_Boards/bull_board"), this_object());

    ob = clone_object("obj/Quest/quest_obj");	
    ob->set_name("orc_slayer");
    ob->set_hint(
		"Retrieve the Orc slayer from the evil orc shaman, and give it to Leo.\n"
	);
    move_object(ob, "room/Church/quest_room");
}
...
ONE_EXIT("room/Village/Road2", "north",
...
int advance(string arg)
{
    ...
    get_level(level);
    if (next_level == 20 && "room/Church/quest_room"->count())
	return 1;
    if (level == 0)
	next_exp = exp;
...
int south() {
    ...
    this_player()->move_player("south#room/AdventurersGuild/adv_inner");
    return 1;
}
...
int list_quests(string num)
{
	...
	"room/Church/quest_room"->list(qnumber);
    else
	"room/Church/quest_room"->count();
    return 1;
}
